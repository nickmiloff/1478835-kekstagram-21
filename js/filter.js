...

{
  ...
  const filtersFunctionsObj = {
    ...
    discussed() {
      return this.getDefaultArr().sort((a, b) => b.comments.length - a.comments.length);
    },
    getDefaultArr: () => {
  ...
  const cahangeActiveButton = window.debounce((buttonNode, filterName) => {
  ...
  filtersFormNode.addEventListener(`click`, (evt) => {
    if (evt.target.classList.contains(`img-filters__button`)) {
      ...
      cahangeActiveButton(evt.target, filterName);
    }
  });
}
